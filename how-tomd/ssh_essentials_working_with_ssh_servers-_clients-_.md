...
When a client connects to the host, wishing to use SSH key authentication, it will inform the server of this intent and will tell the server which public key to use. The server then check its authorized\_keys file for the public key, generate a random string and encrypts it using the public key. This encrypted message can only be decrypted with the associated private key. The server will send this encrypted message to the client to test whether they actually have the associated private key.
...
`~/.ssh/id_rsa`: The private key. DO NOT SHARE THIS FILE! `~/.ssh/id_rsa.pub`: The associated public key. This can be shared freely without consequence. Generate an SSH Key Pair with a Larger Number of Bits
...
Overwrite \(y/n\)? If you choose "yes", your previous key will be overwritten and you will no longer be able to log into servers using that key. Because of this, be sure to overwrite keys with caution.
...
ssh username@remote\_host Copying your Public SSH Key to a Server Without SSH-Copy-ID
...
Basic Connection Instructions The following section will cover some of the basics about how to connect to a server with SSH.
...
To do this, connect to your remote server and open the /etc/ssh/sshd\_config file with root or sudo privileges:
...
To change the port that the SSH daemon listens on, you will have to log into your remote server. Open the sshd\_config file on the remote system with root privileges, either by logging in with that user or by using sudo:
...
This can be accomplished through the root user's authorized\_keys file, which contains SSH keys that are authorized to use the account.

Add the key from your local computer that you wish to use for this process \(we recommend creating a new key for each automatic process\) to the root user's authorized\_keys file on the server. We will demonstrate with the ssh-copy-id command here, but you can use any of the methods of copying keys we discuss in other sections:

```text
ssh-copy-id root@remote_host
```

Now, log into the remote server. We will need to adjust the entry in the authorized\_keys file, so open it with root or sudo access:
...
PermitRootLogin forced-commands-only Save and close the file. Restart the SSH daemon to implement your changes.
...
To enable this functionality, log into your remote server and edit the sshd\_config file as root or with sudo privileges:
...
Inside, you can define individual configuration options by introducing each with a Host keyword, followed by an alias. Beneath this and indented, you can define any of the directives found in the ssh\_config man page:
...
If one does not already exist, at the top of the file, define a section that will match all hosts. Set the StrictHostKeyChecking directive to "no" to add new hosts automatically to the known\_hosts file. Set the UserKnownHostsFile to /dev/null to not warn on new or changed hosts:
